...
function SharePage() {
  ...
  const handleFileSelected = (browserFile: File) => {
    const name = browserFile.name;
    const sizeBytes = browserFile.size;
    const type = browserFile.type || "application/octet-stream";
  ...
  const handleStartSharing = () => {
    ...
    setTimeout(() => setEncryptionStep(1), 600); // AES-256 key generation
    setTimeout(() => setEncryptionStep(2), 1400); // Local Client-side Encryption
    setTimeout(() => setEncryptionStep(3), 2200); // Compliance Policy verification
    setTimeout(() => {
      ...
      const localLink = `${window.location.origin}/share/${randomId}`;
      setShareLink(localLink);
    }, 3000);
  };
