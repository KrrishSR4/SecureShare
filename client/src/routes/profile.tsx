    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-ink selection:text-background font-sans">
      ...
      <main className="pt-24 pb-16 px-6 md:px-10 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="mb-10">
            <h1 className="font-display text-4xl tracking-tight text-foreground">Profile Settings</h1>
            <p className="text-muted-foreground mt-2 text-lg">Manage your personal information and security preferences.</p>
          </div>
          ...
          <div className="grid gap-8 md:grid-cols-3">
            <div className="md:col-span-1">
              <div className="flex flex-col items-center justify-center p-6 border border-border rounded-2xl bg-card">
                ...
                <p className="text-muted-foreground text-sm">{user.role === "ADMIN" ? "Administrator" : "Standard User"}</p>
              </div>
            </div>
                  ...
                  <div className="flex items-center justify-between p-4 border border-border rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{user.oauthProvider === "google" ? "Connected" : "Not connected"}</p>
                      </div>
                    </div>
                    {user.oauthProvider === "google" ? (
                      <span className="text-xs font-semibold text-signal bg-signal/10 px-3 py-1 rounded-full">Connected</span>
                    ) : (
                      <button className="text-xs font-semibold text-ink bg-muted px-3 py-1.5 rounded-lg hover:bg-border transition-colors">Connect</button>
                    )}
                  </div>
                  <div className="flex items-center justify-between p-4 border border-border rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{user.oauthProvider === "github" ? "Connected" : "Not connected"}</p>
                      </div>
                    </div>
              ...
              <div className="border border-border rounded-2xl bg-card p-6 md:p-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Change your account password to maintain security.</p>
                  </div>
                  <button
                ...
                {user.oauthProvider && (
                  <p className="text-xs text-muted-foreground mt-4">
                    You logged in using {user.oauthProvider}. Passwords cannot be changed for OAuth accounts.
                  </p>
                )}
              </div>
            </div>
